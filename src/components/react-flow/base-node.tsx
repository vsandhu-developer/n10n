...
export const BaseNode = forwardRef<HTMLDivElement, BaseNodeProps>(
  ({ className, status, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "relative rounded-sm border-muted-foreground border bg-card text-card-foreground",
        "hover:ring-1",
        className
      )}
      tabIndex={0}
    ...
    >
      ...
      {status === "loading" && (
        <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 animate-spin text-blue-700 stroke-3" />
      )}
    </div>
  )
);
